...
local bgRedWidth = display.viewableContentWidth -- 自動取得螢幕寬度
local bgRedHeight = display.viewableContentHeight -- 自動取得螢幕高度
-- 背景圖片 backgroundRed 參數設定
...
-- 女人圖片
local imgWoman = imgWoman( imgWomanPathName , imgWomanWidth , imgWomanHeight )
...
-- 右邊邊娃娃圖片
local imgDollBigRight = imgDollBigRight( imgDollBigRightPathName , imgDollBigRightWidth , imgDollBigRightHeight )
...
local whatGood = { smallGood , good , veryGood } -- 將 function 的圖片函式名稱儲存在table容器中，不需要()
local rdm = math.random( #whatGood ) -- 使用隨機取值的方式來選擇 whatGood 的元素
whatGood[rdm]() -- 在手機上隨機顯示圖片
