...
funInit = function ()
	...
	-- 哥布林logo圖片
	bgImg = funImages( bgGroup , goblinLogo , 200 , 120 )
	bgImg.x , bgImg.y = _SCREEN.CENTER.X , 70

	-- 帳號輸入欄位底色
	rectUser = funRect( bgGroup , 255 , 160 , 200 , 40 )
	rectUser:setFillColor( 188/255 , 217/255 , 121/255 )

	-- 生成帳號文字
	fontUser = funFont( fontGroup , "帳號" , 120 , 160 , native.systemFont , 30 )
	fontUser:setFillColor( 106/255 , 185/255 , 179/255 )

	-- 密碼輸入欄位底色
	rectPass = funRect( bgGroup , 255 , 210 , 200 , 40 )
	rectPass:setFillColor( 188/255 , 217/255 , 121/255 )

	-- 生成密碼文字
	fontPass = funFont( fontGroup , "密碼" , 120 , 210 , native.systemFont , 30 )
	fontPass:setFillColor( 106/255 , 185/255 , 179/255 )

	-- 生成帳號文字輸入欄位
	inputUser = funInput( 255 , 160 , 200 , 40 ) -- 生成輸入框
	inputUser.hasBackground = false -- 是否顯示欄位底圖
	...
	-- 生成密碼文字輸入欄位
	inputPass = funInput( 255 , 210 , 200 , 40 )
...
-- 分隔線｜偵聽按鈕點擊觸發函式 ----------------------------------------------
--TextField和Button都用到這個Handler，但沒做好區分，會很慘
funBtnOnClick = function( event )
	if (event.target.id == "logBtn") then
		
	elseif (event.target.id == "inputUser") then

	end

	if ( event.phase == "began" ) then
        -- User begins editing "defaultField"
        playAudio = funSound( btnAudioPath , 1 ) --只有載入到記憶體，不會有聲音

        print( "點擊" )

    elseif ( event.phase == "ended" or event.phase == "submitted" ) then
        -- Output resulting text from "defaultField"
        -- local result = chkPwd(username , pwd)
        -- if (result == true) then
        -- 	--通過
        -- else 
        -- 	--失敗
...
funImages = function( _group , _path , _width , _height , _x , _y )
	local img = display.newImageRect( _group , _path , _width , _height )
	img.x , img.y = _x , _y
	return img
end

-- 分隔線｜將讀取音效的方式放入函示方便調用 ---------------------------------------------------
funSound = function( _path , _channel )
	local btnSound = audio.loadSound( _path , { channel = _channel } )
	return btnSound
end

-- 分隔線｜將繪製圖型的方式放入函示方便調用 ---------------------------------------------------
funRect = function( _group , _x , _y , _width , _height )
	local rect = display.newRect( _group , _x , _y , _width , _height )
	return rect
end

-- 分隔線｜將生成文字的方式放入函示方便調用 ---------------------------------------------------
funFont = function( _group , _text , _x , _y , _font , _size )
	local text = display.newText( _group , _text , _x , _y , _font , _size )
	return text
end

-- 分隔線｜將生成輸入欄位的方式放入函示方便調用 ---------------------------------------------------
funInput = function( _x , _y , _width , _height )
	local from = native.newTextField( _x , _y , _width , _height )
	return from
end
