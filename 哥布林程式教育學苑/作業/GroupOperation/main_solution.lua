...
-- 使用函式引入圖片，並給予參數做圖片設定
function makeUiImg( _parent , _path , _width , _height , _x , _y )
	local img = display.newImageRect( _parent, _path , _width , _height )
	img.x , img.y = _x , _y
	return img
end

-- 使用函式引入音效
function makeSound( _path , _channel , completeFun)
	return audio.play( _path , { channel = _channe , onComplete = completeFun } )
end

function onCompleteAudio( e )
	print("onCompleteAudio")
	donRePlay = false;
end
-- 手機圖片 寬 高 X Y 參數設定
...
function makeAudioPlay( e )
	...
	elseif ( e.phase == "ended" ) then
		print("donRePlay" , donRePlay)
		if ( donRePlay == false ) then
			local audioHandle = makeSound( varMusicG , 2 , onCompleteAudio )
			donRePlay = true
		end
	end
end
...
print("audioButtonPlay id:" , audioButtonPlay.id)
-------------------------------------------------------------------------------------
...
local threeImg1 = makeUiImg( threeImgGroup , varThreeImg1 , 47 , 48 )
