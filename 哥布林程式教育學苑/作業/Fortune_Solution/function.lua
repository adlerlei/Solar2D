...
-- 時間：20160720
-----------------------------------------------------------------------------------------
local bigGoodMusic = audio.loadStream("music/coarse_laughing.mp3")
local goodMusic = audio.loadStream("music/devil_laughing1.mp3")
local smallGoodMusic = audio.loadStream("music/monster_grief.mp3")
...
function playMusic( _music , _channel )
	audio.play( _music , { channel = _channel } )
end
...
function makeDollBigLeft()
	local imgDollBigLeft = display.newImageRect( "images/dollBigLeft.png" ,100 , 80 )
	imgDollBigLeft.x , imgDollBigLeft.y = 130 , 200
	return imgDollBigLeft
end
...
function makeImageRect( _pathName , _width , _height)
...
goods = {
	...
	{
		text = "小吉",
		music = smallGoodMusic,
		fun = smallGood,
	}
}

function pickGood()
	local good = math.random( #goods ) -- 使用隨機取值的方式來選擇 whatGood 的元素
	local whatGood = goods[good]
	whatGood.fun() -- 在手機上隨機顯示圖片
	playMusic(whatGood.music , 2 )
end
