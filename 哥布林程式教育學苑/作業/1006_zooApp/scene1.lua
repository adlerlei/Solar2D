...
updateTableRows = function()
	...
	for i=1,rowNum do
		...
		if (row) then
			...
			local img
			if (checkFile(i .. ".png")) then
				-- system.TemporaryDirectory 表示系統暫存路徑檔案
				img = display.newImageRect( i .. ".png", system.TemporaryDirectory , 25, 25 )
			else 
				--當網路圖片不存在，使用預設圖片
				img = display.newImageRect( "images/animal.jpeg" , 25, 25 )
			end
...
onRowRender = function( e )
	...
	-- 儲存網路json圖片資料
	if (not checkFile(index .. ".png")) then
		display.loadRemoteImage( data.A_Pic01_URL , "GET" , remoteImagesResponse , index ..".png" , system.TemporaryDirectory , rowX , rowY )
	else
		updateTableRows()
	end
