...
-- 哥布林人物
function goBlin_1()
	local img = display.newImageRect( "images/goblin_1.png" , 110 , 100 )
	img.x , img.y = 40 , 480
...
function goBlin_2()
	local img = display.newImageRect( "images/goblin_2.png" , 110 , 100 )
	img.x , img.y = 100 , 380
	return img
end

function goBlin_3()
	local img = display.newImageRect( "images/goblin_3.png" , 110 , 100 )
	img.x , img.y = 230 , 350
	img.rotation = 20
	return img
end
...
function goBlin_4()
	local img = display.newImageRect( "images/goblin_4.png" , 190 , 120 )
	img.x , img.y = 200 , 470
	return img
end
-- END
...
-- 背景音樂
function playBgMusic( handle , _channel)
	audio.play( handle , { channel = _channel } ) -- 指定背景音樂佔用第一聲到
	-- audio.fadeOut( { time = 9000 } ) -- 指定背景音樂 淡入 效果三秒
end

function oldPlayBgMusic()
	local bgMusic = audio.loadSound( "music/bgMusic.mp3" ) -- 匯入音樂檔案，附值給變數 bgMusic
	audio.play( handle , { channel = 1 } ) -- 指定背景音樂佔用第一聲到
	-- audio.fadeOut( { time = 9000 } ) -- 指定背景音樂 淡入 效果三秒
end
--END
